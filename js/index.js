const socketIO = io("http://35.229.75.188:3000/");
var user = { id: "", name: "" };
var tabUrl;
var lastSenderId;
...
socketIO.on("olderChatMessages", (messages) => {
  ...
  var lastMessageInserted = document
    .getElementById("messages")
    .getElementsByTagName("li")[messages.length - 1];
  removeLoadingOlderMessages();
...
function changeUserCounter(userCount) {
  document.getElementById("room-users-counter").innerHTML =
    userCount + " user(s)";
}
  ...
  chrome.storage.sync.get("user", (items) => {
    if (items.user) {
      ...
      document.getElementById("welcome-message").innerHTML =
        (user.name ? "Hey " : "Hey user ") + userDiv.outerHTML;
      setUserDivClickable();
    } else {
...
function changeUserName(userId, userName) {
  if (user.id === userId) {
    ...
    document.getElementById("welcome-message").innerHTML =
      "Hey " + userDiv.outerHTML;
    setUserDivClickable();
  }

  var senders = document
    .getElementById("messages")
    .getElementsByClassName("message-sender");
  Array.prototype.forEach.call(senders, (sender) => {
    ...
    if (senderIdDiv.innerHTML.startsWith(userId)) {
      ...
      var senderNameDiv = sender.getElementsByClassName(
        "message-sender-name"
      )[0];
...
function changeMyUserName() {
  ...
  if (!name) {
    document.getElementById("name-input-messge").innerHTML =
      "You should write your name in the input field above!";
    return;
  }
...
function showLoadingOlderMessages() {
  document
    .getElementById("loading-older-messages-wrapper")
...
document.getElementById("messages").onscroll = (event) => {
  if (document.getElementById("messages").scrollTop === 0) {
    showLoadingOlderMessages();
    socketIO.emit("loadOlderMessages", {
      room: tabUrl,
